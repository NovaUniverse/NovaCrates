			...
			if (create != null) {
				...
				if (item != null) {
					if (item.getType() != Material.AIR) {
						create.setIcon(item.getType());

						try {
							create.save();
							sender.sendMessage(ChatColor.GREEN + "The key of " + ChatColor.AQUA + create.getName() + ChatColor.GREEN + " is now " + ChatColor.AQUA + item.getType().name());
						} catch (IOException e) {
